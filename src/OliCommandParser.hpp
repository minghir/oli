...
public:
    static std::vector<std::wstring> tokenize(const std::wstring& line) {
        ...
        for (size_t i = 0; i < line.size(); ++i) {
            ...
            if (c == L'\\') {
                ...
                if (i + 1 < line.size()) {
                    tok += line[i + 1];
                    i++; // Sărim peste caracterul escapat
                }
                continue;
            }
            ...
            // =================================================================
            // Dacă am ajuns aici și caracterul este '"', este garantat o ghilimele de
            // delimitare de string (deschidere sau închidere), nu una escapată!
            if (c == L'"') {
                inQuotes = !inQuotes;
                tok += c;
                if (!inQuotes) flush();
    ...
    static ShellCommand parse(const std::wstring& line) {
        ...
        // Verificăm validitatea doar pentru a seta flag-ul isValid
